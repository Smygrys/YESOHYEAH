...
// Single source of truth: how much 1 unit of each currency is worth in USD.
// Every pairwise rate is DERIVED from this table (see getExchangeRate below),
// which guarantees conversions are always mathematically consistent —
// converting A -> B -> A always returns exactly the amount you started with.
//
// (The old version had 56 independently hand-typed rates that didn't agree
// with each other, e.g. USD->EUR was 0.86 but EUR->USD was 1.09, instead of
// being exact inverses. That mismatch silently destroyed money every time
// setCurrency() ran, since it actually converts and overwrites all stored
// balances, goals, and transactions.)
const USD_BASE_RATES = {
  USD: 1,
  EUR: 1 / 0.86, // ≈ 1.1628 USD per EUR
  PLN: 1 / 3.64, // ≈ 0.2747 USD per PLN
  GBP: 1 / 0.75, // ≈ 1.3333 USD per GBP
  JPY: 1 / 158.15, // ≈ 0.006323 USD per JPY
  CHF: 1 / 0.8, // ≈ 1.25 USD per CHF
  CAD: 1 / 1.39, // ≈ 0.7194 USD per CAD
  AUD: 1 / 1.49, // ≈ 0.6711 USD per AUD
};
...
function getExchangeRate(fromCurrency, toCurrency) {
  ...
  const fromUSD = USD_BASE_RATES[fromCurrency];
  const toUSD = USD_BASE_RATES[toCurrency];

  if (!fromUSD || !toUSD) return 1;

  // amount_in_from * fromUSD = amount_in_USD
  // amount_in_USD / toUSD  = amount_in_to
...
function nextOnboardingStep() {
  if (currentOnboardingStep < 3) {
    ...
    if (currentOnboardingStep === 3) {
      document.getElementById("onboardingIncomeLabel").textContent =
        `Daily Income (${selectedOnboardingCurrency})`;
    }
  }
}
...
// ==================== CURRENCY ====================
function convertStoredMoney(fromCurrency, toCurrency) {
  if (fromCurrency === toCurrency) return;

  const convert = (value) => {
    const number = Number(value);
    return Number.isFinite(number)
      ? convertCurrency(number, fromCurrency, toCurrency)
      : 0;
  };

  // Convert permanent values only.
  appData.dailyIncome = convert(appData.dailyIncome);

  appData.goals.forEach((goal) => {
    goal.target = convert(goal.target);
    goal.current = convert(goal.current);
  });

  appData.transactions.forEach((transaction) => {
    transaction.amount = convert(transaction.amount);

    // Keep the original foreign income unchanged.
    // It is only displayed as a reference.
  });

  appData.stats.totalSavedAllTime = convert(
    appData.stats.totalSavedAllTime || 0,
  );

  appData.stats.totalSpentAllTime = convert(
    appData.stats.totalSpentAllTime || 0,
  );

  // Do not reset savings, balance, or income.
  // updateUI() calculates these from the transactions.
}

function setCurrency(currency) {
  const oldCurrency = appData.settings.currency;

  if (!CURRENCIES[currency] || oldCurrency === currency) return;

  convertStoredMoney(oldCurrency, currency);
  ...
  showToast(
    "success",
    "Currency Changed",
    `Your balance is now shown in ${CURRENCIES[currency].name}`,
  );
}
...
function updateStreak() {
  const savingDays = new Set(
    appData.transactions
      ...
      .map((transaction) => getLocalDateKey(new Date(transaction.date))),
  );
  ...
  // A streak is alive if the user saved today or yesterday.
  if (!savingDays.has(todayKey) && !savingDays.has(yesterdayKey)) {
    appData.stats.streak = 0;
    appData.stats.lastSaveDate = null;
    saveData();
    updateStreakDisplays();
    return;
  }

  // Continue from today, or from yesterday if today's saving has not happened.
  const streakDate = new Date(savingDays.has(todayKey) ? today : yesterday);

  let streak = 0;

  while (savingDays.has(getLocalDateKey(streakDate))) {
    streak++;
    streakDate.setDate(streakDate.getDate() - 1);
  }

  appData.stats.streak = streak;
  appData.stats.bestStreak = Math.max(appData.stats.bestStreak || 0, streak);

  const latestSaving = [...appData.transactions]
    .filter((transaction) => transaction.type === "saving")
    .sort((a, b) => new Date(b.date) - new Date(a.date))[0];

  appData.stats.lastSaveDate = latestSaving ? latestSaving.date : null;
...
function updateStreakDisplays() {
  ...
  const bestStreak = appData.stats.bestStreak || 0;

  const welcomeStreak = document.getElementById("welcomeStreak");
  if (welcomeStreak) {
    welcomeStreak.textContent = `${streak} day${streak === 1 ? "" : "s"}`;
  }

  const analyticsStreak = document.getElementById("analyticsStreak");
  if (analyticsStreak) {
    analyticsStreak.textContent = `${streak} day${streak === 1 ? "" : "s"}`;
    analyticsStreak.title = `Best streak: ${bestStreak} days`;
  }
}
...
function openSpendModal() {
  ...
  document.getElementById("spendCurrencyHint").textContent =
